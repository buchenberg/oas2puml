                    ...
                    (entry, index) => (
                        <div key={`entry-div-${index}`}>
                            ...
                            >
                                ...
                                />
                                <ListItemText
                                    primary={`${entry.request.method} ${this.pathFromUrl(entry.request.url)}`}
                                    secondary={this.hostFromUrl(entry.request.url)} />
                                {/* <form style={styles.container} noValidate autoComplete="off">
                                    <TextField
                                        id={`actor-field-${index}`}
                                        label="Participant Name"
                                        defaultValue={this.resourceFromPath(this.pathFromUrl(entry.request.url)) + " Service"}
                                        style={styles.textField}
                                        helperText="e.g. Some API"
                                        margin="normal"
                                    />
                                    <TextField
                                        ...
                                        style={styles.textField}
                                        label="Resource Name"
                                        defaultValue={this.resourceFromPath(this.pathFromUrl(entry.request.url))}
                                        helperText="e.g. Widgets"
                                        margin="normal"
                                    />
                                </form> */}
                            </ListItem>

                            {(index + 1) < oas.log.entries.length &&
                                <Divider />
                            }
                        </div>
                    ))}
        ...
        return (
                <Card style={styles.card}>
                    <CardHeader
                        ...
                        title="Log Entry Configurator"
                        subheader="Select oas entries to be used for the sequence diagram." />
